...
//        .awaitTermination()
    ...
    val PX  = connDf.withColumn("PX", lit(0))
    val NNP = PX.withColumn("NNP", lit(0))
    val NSP = NNP.withColumn("NSP", lit(0))
    val PSP = NSP.withColumn("PSP", lit(0))
    val IOPR = PSP.withColumn("IOPR", lit(0))
    val Reconnect = IOPR.withColumn("Reconnect", lit(0))
    val FPS = Reconnect.withColumn("FPS", lit(0))
    val TBT = FPS.withColumn("TBT", lit(0))
    val APL = TBT.withColumn("APL", lit(0))
    val PPS = APL.withColumn("PPS", lit(0.0))
